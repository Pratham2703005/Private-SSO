...
export default function InfiniteMenu({
  ...
  // Extract regUserId from URL path: /u/[regUserId]/connected-apps
  const regUserIdMatch = pathname.match(/\/u\/([^/]+)\//);
  const regUserId = regUserIdMatch ? regUserIdMatch[1] : "";
  ...
  const handleAddDomainClick = useCallback(() => {
    if (regUserId) {
      router.push(`/u/${regUserId}/connected-apps/add-domain`);
    }
  }, [regUserId, router]);

  const handleEditDomainClick = useCallback(() => {
    if (regUserId && activeItem && activeItem.id) {
      router.push(`/u/${regUserId}/connected-apps/edit/${activeItem.id}`);
    }
  }, [regUserId, activeItem, router]);
          ...
          <div className="info-grid">
            <div className="info-field">
              ...
              <span className="value">
                {activeItem.provider || activeItem.title}
              </span>
            </div>
            ...
            {activeItem.domain && (
              <div className="info-field">
                ...
                <DomainLink
                  ...
                  isConnected={activeItem.connected}
                />
              </div>
            )}
              ...
              <span
                className={`status ${
                  activeItem.connected
                    ? "connected text-[#1b5b32]"
                    : "disconnected text-[#8f2a2a]"
                }`}
              >
