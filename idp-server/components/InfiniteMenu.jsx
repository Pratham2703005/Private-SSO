...
export default function InfiniteMenu({
  ...
  const handleMovement = useCallback((isMovingState) => {
    onMovementChange(isMovingState);
  }, [onMovementChange]);
  ...
  useEffect(() => {
    ...
    // Only create sketch if it doesn't exist
    if (!sketchRef.current) {
      ...
      // Update items if sketch exists
      sketchRef.current.items = items;
    }
  ...
  }, [items, scale, handleActiveItem, handleMovement]);
  ...
  return (
    <div className="infinite-menu-container">
      <div className="infinite-menu-background" />
      ...
      {/* Top Right Panel - Domain/Provider Info */}
      {activeItem && (
        <div className={`info-panel-top-right ${isMoving ? 'inactive' : 'active'}`}>
          <div className="info-item">
            <div className="info-panel-title">Account Provider</div>
            <div className="info-panel-value">{activeItem.provider || activeItem.title}</div>
          </div>
          {activeItem.domain && (
            <div className="info-item">
              <div className="info-panel-title">Domain</div>
              <div className="info-panel-value">{activeItem.domain}</div>
            </div>
          )}
          {activeItem.createdAt && (
            <div className="info-item">
              <div className="info-panel-title">Connected From</div>
              <div className="info-panel-value">{formatDate(activeItem.createdAt)}</div>
            </div>
          )}
        </div>
      )}

      {/* Bottom Right Panel - Status/Info */}
      {activeItem && (
        <div className={`info-panel-bottom-right ${isMoving ? 'inactive' : 'active'}`}>
          <div className="info-item">
            <div className="info-panel-title">Status</div>
            <div className="info-panel-value" style={{ color: activeItem.connected ? 'var(--input-focus)' : 'var(--error)' }}>
              {activeItem.connected ? '● Connected' : '● Not Connected'}
            </div>
          </div>
        </div>
      )}
